// copied and adapted from https://www.npmjs.com/package/react-native-filereader
//
// FileReader
//
...
(function () {
  ...
  const { getArrayBufferForBlob } = require("react-native-blob-jsi-helper");

  var fs = require("react-native-blob-util").default.fs,
    mime = require("react-native-mime-types"),
    EventEmitter = require("eventemitter3").EventEmitter;
  // console.log('EventEmitter', new EventEmitter().on)
  ...
  function FileReader() {
    ...
    function createFileStream() {
      ...
      if (file._data) {
        process.nextTick(function () {
          stream.emit(
            "data",
            Buffer.from(getArrayBufferForBlob(file), "ascii")
          );
          stream.emit("end");
        });
        file.stream = stream;
      ...
      if (file.path) {
        // TODO url
        if (!chunked) {
          fs.readFile(file.path, "ascii").then(
            (data) => {
    ...
    function _readFile(file, format, encoding) {
      if (
        !file ||
        !(file.name || file.data) ||
        !(file.path || file.stream || file.buffer || file.data)
      ) {
        throw new Error(
          "react-native-filereader error: cannot read as File: " +
            JSON.stringify(file)
        );
      }
    ...
    function readFile(input, format, encoding) {
      ...
      if (file.path) {
        ...
        fs.stat(path).then(
          ...
          function (err) {
            console.warn(
              "react-native-filereader error, maybe you forget request permission: " +
                err.message
            );
            return;
          }
        ...
        if (file.buffer) {
          ...
          throw new Error(
            "react-native-filereader error: No input, nor stream, nor buffer."
          );
        }
